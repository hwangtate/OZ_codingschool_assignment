...
#main Blueprint: 참가자 정보 입력, 퀴즈 제출, 결과 조회 등을 담당.
#admin Blueprint: 관리자 로그인/로그아웃, 질문 관리, 대시보드 등 관리 기능을 담당.
#데이터베이스 모델: 참가자(Participant), 질문(Question), 퀴즈(Quiz), 관리자(Admin) 모델을 사용하여 데이터베이스 작업 수행.
#Plotly: 데이터를 시각화하여 결과 페이지와 관리자 대시보드에 그래프 표시.
#이 코드는 웹 애플리케이션의 기본 구조와 기능을 명확하게 나누어 관리하고 있으며, Flask와 SQLAlchemy, Plotly를 활용하여 데이터베이스 연동과 데이터 시각화를 구현하고 있습니다.